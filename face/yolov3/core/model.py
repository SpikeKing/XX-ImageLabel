...
from face.yolov3.core.utils import compose
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    feats = K.reshape(
        feats, [-1, grid_shape[0], grid_shape[1], num_anchors, num_classes + 5])  # 数据格式
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    '''
    num_layers = len(anchors) // 3  # default setting
    yolo_outputs = args[:num_layers]  # [(?, ?, ?, 18), (?, ?, ?, 18), (?, ?, ?, 18)]
    y_true = args[num_layers:]  # [(?, 13, 13, 3, 6), (?, 26, 26, 3, 6), (?, 52, 52, 3, 6)]
    anchor_mask = [[6, 7, 8], [3, 4, 5], [0, 1, 2]] if num_layers == 3 else [[3, 4, 5], [1, 2, 3]]
    # 输入的尺寸*32就是原始的尺寸
    ...
    for l in range(num_layers):
        object_mask = y_true[l][..., 4:5]  # 是否含有对象
        ...
        raw_true_wh = K.log(y_true[l][..., 2:4] / anchors[anchor_mask[l]] * input_shape[::-1])  # 图片占比
        raw_true_wh = K.switch(object_mask, raw_true_wh, K.zeros_like(raw_true_wh))  # avoid log(0)=-inf
        box_loss_scale = 2 - y_true[l][..., 2:3] * y_true[l][..., 3:4]  # 控制损失

        # Find ignore mask, iterate over each of batch.
        ignore_mask = tf.TensorArray(K.dtype(y_true[0]), size=1, dynamic_size=True)
        object_mask_bool = K.cast(object_mask, 'bool')  # 对象mask
        ...
        loss += xy_loss + wh_loss + confidence_loss + class_loss  # 全部的Loss值相加
        if print_loss:
